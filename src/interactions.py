...
def hydrogen_bond_energy(molecule1, molecule2):
    ...
    H_atoms_mol1 = [coord for idx, coord in enumerate(molecule1.coordinates) if molecule1.smiles[idx] == 'H']
    ON_atoms_mol2 = [coord for idx, coord in enumerate(molecule2.coordinates) if molecule2.smiles[idx] in ['O', 'N']]
        
    total_energy = 0.0
    for H_coord in H_atoms_mol1:
        for ON_coord in ON_atoms_mol2:
            distance = sum((x2 - x1)**2 for x1, x2 in zip(H_coord, ON_coord))**0.5
...
def ionic_interaction_energy(molecule1, molecule2):
    ...
    for H_coord in H_atoms_mol1:
        for ON_coord in ON_atoms_mol2:
            distance = sum((x2 - x1)**2 for x1, x2 in zip(H_coord, ON_coord))**0.5 + epsilon
