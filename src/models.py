...
class FreeEnergyPerturbation:
    ...
    def softened_potential_energy(self, lambda_value, max_energy=1e5):
        ...
        """
        energy = advanced_potential_energy(self.molecule_initial, self.molecule_final)
    ...
    def compute(self, lambda_windows=10, md_steps_per_window=10, T=298.15):
        ...
        lambda_values = [i/lambda_windows for i in range(1, lambda_windows + 1)]
        ...
        for lambda_val in lambda_values:
            ...
            alchemical_molecule.coordinates = [self.molecule_initial.centroid()]
            
            # Calculate energies for each MD step
            energies = [self.softened_potential_energy(lambda_val) for _ in range(md_steps_per_window)]
...
class AdvancedSampler:
    ...
    def replica_exchange(self, temperatures, md_steps_per_temperature):
        ...
        for T in temperatures:
            fep_calculator = FreeEnergyPerturbation(self.molecule_initial, self.molecule_final)
            energy = fep_calculator.compute(T=T, md_steps_per_window=md_steps_per_temperature)
    ...
    def metadynamics(self, collective_variables, biasing_factor):
        ...
        # Very basic
        energy = advanced_potential_energy(self.molecule_initial, self.molecule_final)  # To be imported
