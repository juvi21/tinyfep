...
def main(args):
    ...
    sampled_energy = sampler.sample(
        temperatures=[298.15, 310.15], md_steps_per_temperature=10)
    print(f"Energy with Advanced Sampling: {sampled_energy} J/mol")

    # Apply pressure effects
    energy_with_pressure = PressureEffect.apply_pressure_effect(
        sampled_energy, 1e5)  # Applying 1 atm pressure
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Process SMILES strings of molecules.')
    parser.add_argument('--smiles', type=str, nargs=2, required=True,
                        help='SMILES strings of the two molecules.')
